...
# In api_ollama_integration.py
    ...
    **CONVERSATIONAL BOOKING FLOW (BUTTON-DRIVEN):**
    ...
    - **Step 1: Ask for Specialty.** Your first response MUST ask for the doctor type and provide buttons for each specialty.
      `{"response": "Of course! I'd be happy to help you book an appointment. What type of doctor are you looking for?", "action": "CONVERSATIONAL_BOOKING", "parameters": {"step": "ask_specialty"}, "interactive_buttons": [{"text": "General Physician", "action": "SELECT_SPECIALTY"}, {"text": "Child Specialist", "action": "SELECT_SPECIALTY"}, {"text": "Dermatologist", "action": "SELECT_SPECIALTY"}]}`
    - **Step 2: Show Doctors & Ask for Choice.** After the user selects a specialty, you will be given a list of real doctors in a CONTEXT message. Your response MUST present these doctors as buttons.
      `{"response": "Here are the available General Physicians:", "action": "CONVERSATIONAL_BOOKING", "parameters": {"step": "ask_doctor"}, "interactive_buttons": [{"text": "Dr. Aarav Sharma", "action": "SELECT_DOCTOR"}]}`
    - **Step 3: Ask for Date.** After the user selects a doctor, you will be given a list of available dates in a CONTEXT message. Your response MUST present these dates as buttons.
      `{"response": "Great. When would you like to schedule the appointment?", "action": "CONVERSATIONAL_BOOKING", "parameters": {"step": "ask_date"}, "interactive_buttons": [{"text": "Today", "action": "SELECT_DATE"}, {"text": "Tomorrow", "action": "SELECT_DATE"}]}`
    - **Step 4: Show Time Slots & Ask for Choice.** After the user selects a date, you will be given a list of available time slots in a CONTEXT message. Your response MUST present these as buttons.
      `{"response": "Perfect. Here are the available time slots for that day:", "action": "CONVERSATIONAL_BOOKING", "parameters": {"step": "ask_time"}, "interactive_buttons": [{"text": "07:30 PM", "action": "SELECT_DATETIME"}, {"text": "08:30 PM", "action": "SELECT_DATETIME"}]}`
    - **Step 5: Confirm & Book.** After the user chooses a time, your action MUST be `FINALIZE_BOOKING`.
      `{"response": "Got it! I am confirming your appointment with Dr. Aarav Sharma for 07:30 PM.", "action": "FINALIZE_BOOKING", "parameters": {"doctorName": "Aarav Sharma", "dateTimeISO": "2025-10-15T19:30:00"}, "interactive_buttons": []}`"""
