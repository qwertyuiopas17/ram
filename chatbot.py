    ...
    try:
        ...
        if is_booking_flow:
            ...
            if not last_step or nlu_understanding.get('primary_intent') == 'appointment_booking' and last_step != 'finalize':
                ...
                ai_message_override = "i need to book appointment"
                ...
                else: # No doctors found, restart flow
                ...
                selected_date_iso = next((btn.get('parameters', {}).get('dateISO') for btn in last_bot_response.get('interactive_buttons', []) if btn.get('text') == user_message), datetime.now().strftime('%Y-%m-%d'))
                booking_context['date'] = selected_date_iso
                
                # FIX: Generate predictable, non-relative time slots to avoid timezone issues
                slots = []
                ...
                booking_context['time_iso'] = next((btn.get('parameters', {}).get('dateTimeISO') for btn in last_bot_response.get('interactive_buttons', []) if btn.get('text') == user_message), None)
                
                # ADDED: The missing "mode" selection step
                modes = ["Video Call", "Audio Call", "Photo-based"]
                mode_buttons = ', '.join([f'{{"text": "{mode}", "action": "SELECT_MODE"}}' for mode in modes])
                ...
                booking_context['last_step'] = 'ask_mode'
                 ...
                 booking_context['mode'] = user_message
