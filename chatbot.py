...
def predict():
    """
    Enhanced prediction endpoint with robust, stateful conversational booking support.
    """
    try:
        ...
        if task.get('task') == 'appointment_booking' or nlu_understanding.get('primary_intent') == 'appointment_booking':
        ...
        if is_booking_flow:
            ...
            if not last_step or nlu_understanding.get('primary_intent') == 'appointment_booking' and last_step != 'finalize':
                ...
                # Generate specialty selection buttons
                specialty_buttons = ', '.join([
                    '{"text": "General Physician", "action": "SELECT_SPECIALTY", "parameters": {"specialty": "general"}}',
                    '{"text": "Dermatologist", "action": "SELECT_SPECIALTY", "parameters": {"specialty": "dermatologist"}}',
                    '{"text": "Child Specialist", "action": "SELECT_SPECIALTY", "parameters": {"specialty": "child_specialist"}}'
                ])
                ...
                if hasattr(data, 'get') and data.get('buttonAction') == 'SELECT_SPECIALTY':
                    # This is a button click
                    selected_specialty = data.get('parameters', {}).get('specialty')
                else:
                    # This is text input - use the user's message
                    selected_specialty = user_message

                if selected_specialty:
                    booking_context['specialty'] = selected_specialty
                    doctors = Doctor.query.filter(Doctor.specialization.ilike(f'%{selected_specialty}%'), Doctor.is_active == True).all()
                    if doctors:
                ...
                if hasattr(data, 'get') and data.get('buttonAction') == 'SELECT_DOCTOR':
                    # This is a button click
                ...
                if hasattr(data, 'get') and data.get('buttonAction') == 'SELECT_DATE':
                    # This is a button click
                ...
                for i in range(4):
                    slot_time = datetime.fromisoformat(selected_date_iso).replace(hour=base_hour + i, minute=0, second=0, microsecond=0)
                ...
                if hasattr(data, 'get') and data.get('buttonAction') == 'SELECT_TIME':
                    # This is a button click
                ...
                selected_mode = None
                if hasattr(data, 'get') and data.get('buttonAction') == 'SELECT_MODE':
                    # This is a button click
        ...
        if not action_payload:
            ...
            # For appointment booking, ensure fallback generates proper buttons
            if current_task.get('task') == 'appointment_booking':
                booking_context = current_task.get('context', {})
                last_step = booking_context.get('last_step')

                if last_step == 'ask_specialty':
                    # Generate doctor specialty buttons
                    action_payload['response'] = "What type of doctor are you looking for?"
                    action_payload['action'] = 'SELECT_SPECIALTY'
                    ...
                    # Generate doctor selection buttons
                    ...
                    if doctors:
                        action_payload['response'] = f"Please select a {specialty} doctor:"
                        action_payload['action'] = 'SELECT_DOCTOR'
                        action_payload['interactive_buttons'] = [
                            {'text': f'Dr. {doc.full_name}', 'action': 'SELECT_DOCTOR', 'parameters': {'doctor_name': doc.full_name}}
                            for doc in doctors[:5]  # Limit to 5 doctors
                        ]
...
def conversational_booking():
    ...
    try:
        ...
        # Get booking data from database instead of using hardcoded data
        try:
            booking_response = get_booking_data_internal()
            if booking_response and booking_response.get("success"):
                website_booking_data = booking_response["booking_data"]
                logger.info(f"Using dynamic booking data: {len(website_booking_data.get('categories', []))} categories, {sum(len(doctors) for doctors in website_booking_data.get('doctors', {}).values())} doctors")
            else:
                logger.warning("Failed to get dynamic booking data, falling back to hardcoded data")
