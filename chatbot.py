            ...
            try:
                ...
                if bot_action == 'CONVERSATIONAL_BOOKING':
                    if bot_step == 'ask_specialty':
                        # The user's message IS the specialty. Find doctors for it.
                        ...
                        if doctors:
                            ...
                            ai_message_override = f"CONTEXT: The user chose '{specialty}'. NOW, ask the user to choose one of the following doctors by showing these buttons: [{doctor_buttons}]"
                        else:
                            ai_message_override = "CONTEXT: The user chose a specialty with no available doctors. Apologize and ask them to choose another from the main list."
                    
                    elif bot_step == 'ask_doctor':
                        # The user's message IS the doctor's name. Get time slots.
                        doctor_name = user_message.replace("Dr. ", "").strip()
                        slots = []
                        start_time_dt = datetime.now().replace(second=0, microsecond=0)
                        # Find the next available half-hour slot.
                        if start_time_dt.minute < 30: start_time_dt = start_time_dt.replace(minute=30)
                        else: start_time_dt = (start_time_dt + timedelta(hours=1)).replace(minute=0)
                        
                        # Generate the next 4 available slots.
                        for i in range(4):
                            slot_time = start_time_dt + timedelta(minutes=30 * i)
                            slots.append({"text": slot_time.strftime('%I:%M %p'), "iso": slot_time.isoformat()})

                        time_buttons = ', '.join([f'{{"text": "{s["text"]}", "action": "SELECT_DATETIME", "parameters": {{"dateTimeISO": "{s["iso"]}"}}}}' for s in slots])
                        # Create a specific instruction for the AI.
                        ai_message_override = f"CONTEXT: The user chose '{doctor_name}'. NOW, ask the user to choose a time slot by showing these buttons: [{time_buttons}]"
