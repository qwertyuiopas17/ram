...
def predict():
    ...
    try:
        ...
        if conversation_memory:
            # --- THIS IS THE FIX ---
            # The function expects the entire nlu_understanding object, not individual keys.
            conversation_memory.add_conversation_turn(
                user_id=current_user.patient_id,
                user_message=data.get("message"),
                bot_response=action_payload_str,
                nlu_result=nlu_understanding,  # Pass the whole NLU result here
                action_taken=response_data.get('action') # Pass the final action taken
            )
            # --- END OF FIX ---
