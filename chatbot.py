    ...
    try:
        ...
        if history_turns:
            try:
                ...
                # If the last action was part of our booking flow...
                # In chatbot.py, inside the predict function's orchestration logic

                if bot_action == 'CONVERSATIONAL_BOOKING':
                    # --- THIS IS THE FIX ---

                    if bot_step == 'ask_specialty':
                        # User selected a specialty. Find doctors.
                        ...
                        if doctors:
                            ...
                            ai_message_override = "CONTEXT: The user chose a specialty with no doctors. Apologize and ask them to choose another."

                    elif bot_step == 'ask_doctor':
                        # User selected a doctor. Generate DATE buttons for the next 5 days.
                        doctor_name = user_message.replace("Dr. ", "").strip()
                        dates = []
                        today = datetime.now()
                        for i in range(5):
                            current_date = today + timedelta(days=i)
                            day_name = "Today" if i == 0 else "Tomorrow" if i == 1 else current_date.strftime('%a, %b %d')
                            dates.append({"text": day_name, "iso": current_date.strftime('%Y-%m-%d')})
                        
                        date_buttons = ', '.join([f'{{"text": "{d["text"]}", "action": "SELECT_DATE", "parameters": {{"dateISO": "{d["iso"]}"}}}}' for d in dates])
                        ai_message_override = f"CONTEXT: The user chose '{doctor_name}'. NOW, ask them to select an appointment date by showing these buttons: [{date_buttons}]"

                    elif bot_step == 'ask_date':
                        # User selected a date. Generate TIME slots for that date.
                        selected_date_str = user_message # The frontend will send the button text (e.g., "Today")
                        
                        # Find the ISO date from the last bot response to be precise
                        last_buttons = last_bot_response.get('interactive_buttons', [])
                        selected_date_iso = None
                        for btn in last_buttons:
                            if btn.get('text') == selected_date_str:
                                selected_date_iso = btn.get('parameters', {}).get('dateISO')
                                break
                        
                        if not selected_date_iso:
                            # Fallback if we can't find the date
                             selected_date_iso = datetime.now().strftime('%Y-%m-%d')

                        selected_date = datetime.fromisoformat(selected_date_iso)
                        start_time_dt = datetime.now()
                        
                        # If the selected date is today, start slots from the next available hour.
                        if selected_date.date() == start_time_dt.date():
                            if start_time_dt.minute > 0: # If it's 2:15, start from 3:00
                                start_time_dt = (start_time_dt + timedelta(hours=1)).replace(minute=0)
                        else: # If it's a future date, start from a reasonable morning time.
                            start_time_dt = selected_date.replace(hour=9, minute=0)

                        slots = []
                        for i in range(4): # Generate 4 available slots
                            slot_time = start_time_dt + timedelta(hours=i)
                        ...
                        ai_message_override = f"CONTEXT: The user chose a date. NOW, ask them to choose a time slot by showing these buttons: [{time_buttons}]"
                    # --- END OF FIX ---

            except (json.JSONDecodeError, AttributeError):
                pass # Ignore if parsing fails; proceed normally.
        ...
        if sehat_sahara_client and sehat_sahara_client.is_available:
            ...
            if action_payload_str:
                try:
                    ...
                    action_payload = None # AI failed to return valid JSON.
        ...
        return jsonify({"response": "I'm having a technical issue. Please try again in a moment.", "action": "SHOW_APP_FEATURES", "interactive_buttons": []}), 500
