    ...
    def __init__(self, model_path: str = None, openrouter_api_key: str = None):
        ...
        if self.openrouter_api_key:
            try:
                ...
                if self._test_openrouter_connection():
                    self.use_openrouter = True
                    self.openrouter_available = True
                    self.logger.info("✅ OpenRouter API connected for enhanced multilingual NLU")
                else:
                    self.logger.warning("❌ Invalid OpenRouter API key provided")
    ...
    def _test_openrouter_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.logger.info("✅ OpenRouter API key validated successfully")
                return True
            else:
                self.logger.error(f"❌ OpenRouter API key validation failed: {response.status_code}")
                return False
